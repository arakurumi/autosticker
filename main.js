      ...
      if (!(fromMe)) {
        if (txt.match(/https:\/\/www.instagram.com\/(p|reel|tv)/gi)) {
          await request.get({
            headers: {
              ...
              'apikey': 'ArfinMeka2021'
            },
            url: 'https://api.imjustgood.com/instapost='+body
          }, function (error, response, body) {
            const pp = JSON.parse(body)
            const cok = pp.result.postData
            const coks = pp.result.caption
            for (var ia = 0; ia < cok.length; ia++) {
              if (cok[ia].type === 'video') {
                fn.sendFileFromUrl(from, `${cok[ia].postUrl}`, 'google.mp4', `Deskripsi: ` + coks)
              }
              if (cok[ia].type === 'image') {
                fn.sendFileFromUrl(from, `${cok[ia].postUrl}`, 'google.jpg', `Deskripsi: ` + coks)
              }
            }
          })
        }
        if (txt.includes('tiktok.com')) {
          await request.get({
            ...
            url: 'https://api.imjustgood.com/tiktokdl='+body
          }, function (error, response, body) {
            const pp = JSON.parse(body)
            const cok = pp.result.caption
            const coks = pp.result.no_watermark
            fn.sendFileFromUrl(from, coks, 'google.mp4', `Deskripsi: ` + cok)
          })
        }
